...
def test_depth_4_link_tree_but_depth_is_3():
    with Scenario() as s:
        ...
        tested = crawler.crawl.Crawl('root', 3, ingore_regex=Fake('ingore_regex'), find_urls=Fake('find_links'))
        assert tested.web_of_links() == [
                ['root'],
                ['link1', 'link2'],
                ['link1.1', 'link1.2', 'link2.1'],
...
def test_depth_2_link_tree_with_ingore_regex_start_l():
    with Scenario() as s:
        s.find_links('root') >> ['link1', 'link2']


        tested = crawler.crawl.Crawl('root', 2, "^l", find_urls=Fake('find_links'))
        assert tested.web_of_links() == [
                ['root'],
           ]
...
def test_depth_2_link_tree_with_ingore_regex():
    with Scenario() as s:
        ...
        tested = crawler.crawl.Crawl('root', 2, "^n", find_urls=Fake('find_links'))
        assert tested.web_of_links() == [
                ['root'],
                ['link1', 'link2'],
           ]
...
def test_depth_4_link_tree_with_ingore_regex_start_link1():
    with Scenario() as s:
        s.find_links('root') >> ['link1', 'link2']
        s.find_links('link2') >> ['link2.1']
        s.find_links('link2.1') >> []
...
def test_depth_4_link_tree_with_ingore_regex_start_link12():
    with Scenario() as s:
        s.find_links('root') >> ['link1', 'link2']
        s.find_links('link1') >> ['link1.1', 'link1.2']
        s.find_links('link2') >> ['link2.1']
        ...
        tested = crawler.crawl.Crawl('root', 4, "link2.1", find_urls=Fake('find_links'))
        assert tested.web_of_links() == [
            ['root'],
            ['link1', 'link2'],
            ['link1.1', 'link1.2'],
